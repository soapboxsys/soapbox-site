...
Special care must be given to prevent the introduction of timing side-channel attacks.
...
The exact format of the leaves and branch digests needed to construct the path is as follows:
From [RFC 6962 section 2.1.1](https://tools.ietf.org/html/rfc6962#section-2.1.1) and [bitcoin docs](https://en.bitcoin.it/wiki/Dump_format#CBlock)
...
After recieving a signed JSON response, an API client must check the JWS signature inline with JWS spec.
